...
@section('content')
    <h1 class="font-semibold text-3xl">Tambah Data Siswa</h1>
    <form action="{{ route('siswa.store') }}" method="post" class="mt-5 space-y-4">
        @csrf
        <div>
        ...
        <div>
            ...
            <input type="text" id="nama_sekolah" name="nama_sekolah"
                class="bg-neutral-secondary-medium border border-default-medium text-heading rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                placeholder="Masukkan nama sekolah siswa" value="{{ old('nama_sekolah') }}" required />
        </div>
        <div>
            ...
            <input type="text" id="kelas_siswa" name="kelas_siswa"
                class="bg-neutral-secondary-medium border border-default-medium text-heading rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                placeholder="Masukkan kelas siswa" value="{{ old('kelas_siswa') }}" required />
        </div>
        <div class="mt-5">
            <button type="submit"
